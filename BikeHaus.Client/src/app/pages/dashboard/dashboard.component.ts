...
@Component({
      ...
      <div class="shortcuts-grid">
        ...
        <a routerLink="/bicycles" class="shortcut-card">
          <div class="shortcut-icon-wrap accent-info">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="5.5" cy="17.5" r="3.5"/><circle cx="18.5" cy="17.5" r="3.5"/><path d="M15 6a1 1 0 1 0 0-2 1 1 0 0 0 0 2zm-3 11.5V14l-3-3 4-3 2 3h2"/></svg>
          </div>
          <div class="shortcut-info">
      ...
      <div class="recent-sections">
        <div class="recent-section">
          <div class="section-header">
            ...
            <a routerLink="/purchases" class="view-all">Alle ansehen
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg>
            </a>
          </div>
          <div class="table-wrap" *ngIf="data.recentPurchases?.length; else noPurchases">
            <table>
              <thead>
                <tr>
                  <th>Beleg-Nr.</th>
              ...
              <tbody>
                ...
                >
                  <td><span class="badge-mono">{{ p.belegNummer }}</span></td>
                  <td>{{ p.bikeInfo }}</td>
                  <td>{{ p.sellerName }}</td>
          ...
          <ng-template #noPurchases>
            <div class="empty-state">
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/></svg>
        ...
        <div class="recent-section">
          <div class="section-header">
            ...
            <a routerLink="/sales" class="view-all">Alle ansehen
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg>
            </a>
          </div>
              ...
              <tbody>
                <tr
                  *ngFor="let s of data.recentSales"
                  ...
                  (click)="goToSale(s.id)"
                >
                  <td><span class="badge-mono">{{ s.belegNummer }}</span></td>
                  <td>{{ s.bikeInfo }}</td>
                  <td>{{ s.buyerName }}</td>
                  <td class="price">{{ s.preis | number: '1.2-2' }} €</td>
                  <td class="date">{{ s.verkaufsdatum | date: 'dd.MM.yyyy' }}</td>
                </tr>
              </tbody>
            </table>
          </div>
  ...
  styles: [
    `
      ...
      .accent-primary  { background: rgba(99,102,241,0.1); color: #6366f1; }
      .accent-success  { background: rgba(16,185,129,0.1); color: #10b981; }
      .accent-info     { background: rgba(6,182,212,0.1);  color: #06b6d4; }
      .accent-warning  { background: rgba(245,158,11,0.1); color: #f59e0b; }
      .accent-violet   { background: rgba(139,92,246,0.1);  color: #8b5cf6; }
      .accent-emerald  { background: rgba(52,211,153,0.1);  color: #34d399; }
      .accent-sky      { background: rgba(56,189,248,0.1);  color: #38bdf8; }
      .accent-rose     { background: rgba(244,63,94,0.1);   color: #f43f5e; }
      .accent-amber    { background: rgba(251,191,36,0.1);  color: #fbbf24; }
      .accent-slate    { background: rgba(100,116,139,0.1); color: #64748b; }
