...
6. **DMARC is `p=quarantine`** (`sp=quarantine`, `pct=100`, reports to `postmaster@`) — this file previously claimed `p=reject`; the published record says otherwise. Any new sending domain/subdomain still needs SPF + DKIM aligned before sending, or mail lands in the spam folder.
7. **T-Online blocks unknown sending IPs.** `554 IP=… - None/bad reputation … (NOWL)` is T-Online's whitelist talking, not a spam verdict on the mail — nothing in DNS or the message content fixes it, the reputation has to be reset by T-Online. Runbook and the ready-to-send request: [deploy/MAIL-DELIVERABILITY.md](deploy/MAIL-DELIVERABILITY.md), diagnosis via the `Mailcow T-Online Reputation` workflow. Queued mail expires after 5 days, so `flush-queue` right after the unblock.
8. **Background sync** scrapes Kleinanzeigen via Playwright/Chromium running inside the API container — the runtime image is heavy (~1.5 GB) because of this. Don't strip the system libs from the Dockerfile.
9. **Angular `serve:ssr:BikeHaus.Homepage`** runs the built server bundle. To dev SSR-aware code, build first; for plain SPA dev use `npm start`.
...
14. **Mailcow serves a COPY of the TLS certificate, not certbot's files.** Mailcow's own ACME can never work here — ports 80/443 belong to the shop's nginx, so mailcow's `acme-mailcow` fails HTTP validation and drops a *self-signed* cert into `data/assets/ssl/`, which every mail client rejects. Hence `SKIP_LETS_ENCRYPT=y` in `mailcow.conf` plus [deploy/mailcow-cert-sync.sh](deploy/mailcow-cert-sync.sh), which copies certbot's `fullchain.pem`/`privkey.pem` into mailcow and reloads postfix/dovecot/nginx-mailcow. An hourly systemd timer runs it ([deploy/install-mailcow-cert-sync.sh](deploy/install-mailcow-cert-sync.sh)); `ssl-ops.yml` re-installs and re-enables it daily, so a disabled timer self-heals. Three traps: (a) **`docker compose restart` does NOT re-read env vars** — after changing `SKIP_LETS_ENCRYPT` you must `docker compose up -d acme-mailcow` or the container keeps the old value; (b) a one-off manual copy is a *snapshot* — without the timer the mail ports freeze on the old cert and break ~90 days later while the website stays fine; (c) port 443 being healthy says nothing about ports 465/993 — `deploy/ssl-status.sh` now probes them separately and compares fingerprints against 443.
16. **Prerendering freezes live data — the showroom was stale for weeks.** `CommonEngine.retrieveSSGPage` serves a prerendered `index.html` verbatim, and `provideClientHydration()` ships the HTTP transfer cache, so the browser reuses the API response baked in at build time and never refetches. A route in [prerender-routes.txt](BikeHaus.Homepage/prerender-routes.txt) therefore shows the inventory of the last homepage build, not today's — `/de/showroom` still listed 158 Kleinanzeigen ads (`lastScrapedAt` 2026-09-16) while the API returned `[]`. Fix in [server.ts](BikeHaus.Homepage/server.ts): the showroom is still prerendered (only that way does it carry canonical + hreflang into the build output that `scripts/generate-sitemap.mjs` derives the sitemap from — dropping it from the route list silently costs 4 sitemap URLs), but it is served with `document:` instead of `documentFilePath:`, which makes CommonEngine render fresh. Verify with `ng-server-context`: `ssr` = live, `ssg` = frozen. Any new route that shows inventory needs the same treatment.

15. **TLS certs live in nginx's memory, not on disk.** certbot renews inside its own container; nginx only re-reads the files on reload. The nginx service therefore runs a 6h self-reload loop (`command:` in docker-compose.yml) — don't remove it, or HTTPS silently dies the next time the in-memory cert expires. Adding a new `server_name` with a 443 block also means adding that host to `DOMAINS` in `deploy/setup-ssl.sh`; otherwise the host gets a wrong-name cert that HSTS (`includeSubDomains` on the apex) makes unbypassable. The reverse bites too: `mail.` was in the live certificate but missing from `DOMAINS`, so running the script would have dropped it. CI enforces the match (`nginx-config-check.yml`). Diagnose with `deploy/ssl-status.sh`, never with reflexive `--force-renewal` (Let's Encrypt: 5 duplicate certs/week).
